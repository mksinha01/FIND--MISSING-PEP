...
export const SightingDetailPage: React.FC = () => {
  ...
  const handleReview = async (action: 'confirm' | 'reject') => {
    ...
    try {
      const updated = action === 'confirm' ? await sightingsApi.confirm(sighting.id, reviewNotes.trim() || undefined) : await sightingsApi.reject(sighting.id, reviewNotes.trim() || undefined);
      setSighting(updated);
      addToast(action === 'confirm' ? 'Sighting verified and confirmed as a MATCH!' : 'Sighting flagged and rejected as false positive.', action === 'confirm' ? 'success' : 'info');
    } catch { addToast(action === 'confirm' ? 'Failed to confirm sighting match' : 'Failed to reject sighting match', 'error'); }
  ...
  return (
    <div className="main-content">
      <div className="page-toolbar"><button type="button" onClick={() => navigate(-1)} className="btn btn-secondary btn-sm"><ArrowLeft size={16} aria-hidden="true" /> Back</button>{person && <button type="button" onClick={() => navigate(`/reports/${person.id}`)} className="btn btn-secondary btn-sm">View Full Case: {person.full_name}</button>}</div>
      ...
      <div className="glass-panel review-card"><h3 className="subsection-title">⚖️ {t.sighting.reviewDecision}</h3>{sighting.status !== 'PENDING' ? <div className={`review-result review-result--${sighting.status === 'CONFIRMED' ? 'success' : 'muted'}`}><strong>{t.sighting.alreadyReviewed.replace('{status}', sighting.status)}</strong>{sighting.review_notes && <p><strong>Review Notes:</strong> {sighting.review_notes}</p>}</div> : <><div className="form-group"><label className="form-label" htmlFor="review-notes">{t.sighting.reviewNotesLabel}</label><textarea id="review-notes" className="form-control" rows={2} placeholder={t.sighting.reviewNotesPlaceholder} value={reviewNotes} onChange={(event) => setReviewNotes(event.target.value)} /></div><div className="review-actions"><button type="button" onClick={() => void handleReview('confirm')} disabled={isSubmittingReview} className="btn btn-success"><CheckCircle size={18} aria-hidden="true" /> {isSubmittingReview ? 'Submitting...' : t.sighting.confirmMatchBtn}</button><button type="button" onClick={() => void handleReview('reject')} disabled={isSubmittingReview} className="btn btn-danger"><XCircle size={18} aria-hidden="true" /> {isSubmittingReview ? 'Submitting...' : t.sighting.rejectMatchBtn}</button></div></>}</div>
